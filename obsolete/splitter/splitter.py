...
if __name__ == '__main__':
	...
	f = open(output_dir+'/split.json', 'w')
	f.write('{\n\t"pipeline": {\n\t\t"name": "'+args.pipeline+'"\n\t},\n');
	f.write('\t"transform": {\n\t\t"cmd": [ "sh" ],\n\t\t"stdin": [\n\t\t\t"split -d -a 10 -l '+str(args.nlines)+' /pfs/'+args.input_repo+'/data /pfs/out/data_"\n\t\t],\n\t\t"image": "fuquanwang/pach-job-shim:latest"\n\t},\n')
	f.write('\t"parallelism_spec": {\n\t\t"strategy": "CONSTANT",\n\t\t"constant": 1\n\t},\n')
	f.write('\t"inputs": [\n\t\t{\n\t\t\t"repo": {\n\t\t\t\t"name": "'+args.input_repo+'"\n\t\t\t}\n\t\t}\n\t]\n}\n')
	f.close()

	subprocess.call(['pachctl', 'create-pipeline', '-f', output_dir+'/split.json'])
